...
9. **Unidades Construtivas Padrão em Obras**: Todo Centro de Custo cadastrado como **Obra** (`obra` ou `centro_custo_obra`) gera automaticamente ao ser salvo as 5 unidades construtivas padrão como nós-folha para apropriação e orçamento: `MÃO DE OBRA`, `IMPOSTOS`, `ENGENHARIA`, `ADMINISTRATIVO` e `DIRETORIA`. Para os demais Centros de Custo (`centro_custo`), as unidades construtivas e subdivisões departamentais são livres para criação manual.
